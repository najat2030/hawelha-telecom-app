...
def extract_numbers(text):
    ...
    # تعديل ذكي لفصل الأرقام مع الحفاظ على إشارة السالب
    text = re.sub(r'(\d)-', r'\1 -', text) 
    # استخراج الأرقام مع علامة السالب إن وجدت
                ...
                for table in tables or []:
                    for i, row in enumerate(table):
                        ...
                        if phone:
                            ...
                            # الضبط النهائي لعمود التسويات (إجباره على السالب لو كان فيه قيمة)
                            taswiya = g(10)
                            ...
                            records.append({
                                ...
                                "رسوم تسويات": taswiya, # القيمة المجبرة على السالب
...
if st.button("🚀 بدء المعالجة والتحليل"):
    if files:
        ...
        if all_data:
            ...
            with m3: st.markdown(f'<div class="metric-card"><div class="metric-title">🧾 تسويات</div><div class="metric-value" style="color: red;">{df["رسوم تسويات"].sum():,.1f}</div></div>', unsafe_allow_html=True)
