            ...
            for table in page.extract_tables() or []:
                ...
                while i < len(table):
                    ...
                    if phone:
                        ...
                        vals = [v for v in vals if str(int(v)) != phone]
                        vals = vals[::-1]
...
def parse_smart_text(file):
    ...
    phones = re.findall(r'01[0125]\d{8}', full_text)
        ...
        def find_value(labels):
            ...
            for label in labels:
                ...
                if match:
                    ...
                    if str(int(val)) == phone:
                        continue
                    return val
            return None

        monthly = find_value(["الرسوم الشهرية"])
        taxes = find_value(["الضرائب"])
        total = find_value(["إجمالي", "القيمة المستحقة"])

        numbers = extract_numbers(full_text)
        numbers = [n for n in numbers if str(int(n)) != phone]

        def fallback(val, idx):
            return val if val is not None else (numbers[idx] if idx < len(numbers) else 0)

        records.append({
            "محمول": phone,
            "رسوم شهرية": fallback(monthly, 0),
            "رسوم الخدمات": 0,
            "مكالمات محلية": 0,
            "رسائل محلية": 0,
            "إنترنت محلية": 0,
            ...
            "إنترنت تجوال": 0,
            "رسوم تسويات": 0,
            "ضرائب": fallback(taxes, -2),
            "إجمالي": fallback(total, -1),
        })
...
# ================= SIGNATURE =================
st.markdown("---")
st.markdown("<div style='text-align:center;'>Developed by Nagat ❤️</div>", unsafe_allow_html=True)
