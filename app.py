...
with st.sidebar:
    ...
    st.markdown("""
    ### 📊 الترتيب النهائي للقيم (من ملف الإكسل المرجعي):
    1. رسوم شهرية (Index 0)
    2. قيمة الضرائب (Index 1)
    3. رسوم وتسويات أخرى (Index 2)
    4. مكالمات محلية / رسائل / إنترنت (Index 3-5)
    5. مكالمات دولية / رسائل (Index 6-7)
    6. مكالمات تجوال / رسائل / إنترنت (Index 8-10)
    7. إجمالي (Index 12)
    """)
    
    st.markdown("---")
    st.info("💡 **ملاحظة:** تم ضبط الترتيب ليطابق ملف الإكسل المرفق تماماً")
...
def extract_values_from_row(row):
    """
    استخراج جميع الأرقام من الصف مع الحفاظ على السالب
    """
    values = []
    if not row:
        return values
...
def create_record(phone, values):
    """
    توزيع القيم على الأعمدة حسب الترتيب الثابت المستخرج من ملف الإكسل المرجعي
    
    الترتيب في ملف الإكسل (من اليسار لليمين في صفوف البيانات):
    Index 0: رسوم شهرية (45.5)
    Index 1: قيمة الضرائب (11.35)
    Index 2: رسوم وتسويات أخرى (0 أو -117)
    Index 3: مكالمات محلية (أو أول عمود فرعي للمحلية)
    Index 4: رسائل محلية
    Index 5: إنترنت محلية
    Index 6: مكالمات دولية
    Index 7: رسائل دولية
    Index 8: مكالمات تجوال
    Index 9: رسائل تجوال
    Index 10: إنترنت تجوال
    Index 11: (غالباً فارغ أو جزء من التسويات، نضعه في تسويات أخرى احتياطياً أو نتجاهله إذا كان صفراً)
    Index 12: إجمالي (56.85)
    """
    ...
    return {
        'محمول': phone,
        # التوزيع الدقيق حسب ملف الإكسل المرفق
        'رسوم شهرية': get_val(0),          
        'قيمة الضرائب': get_val(1),        # <-- التصحيح هنا: الضريبة هي القيمة الثانية
        'رسوم وتسويات اخري': get_val(2),   # <-- التسويات هي القيمة الثالثة
        'مكالمات محلية': get_val(3),       
        'رسائل محلية': get_val(4),         
        'إنترنت محلية': get_val(5),        
        'مكالمات دولية': get_val(6),       
        'رسائل دولية': get_val(7),         
        'مكالمات تجوال': get_val(8),       
        'رسائل تجوال': get_val(9),         
        'إنترنت تجوال': get_val(10),        
        'إجمالي': get_val(12)              # الإجمالي هو القيمة الأخيرة (Index 12)
    }
...
def parse_etisalat_table(table):
    ...
    while i < len(table):
        ...
        if phone_match:
            ...
            values = []
            # البحث عن القيم في الصف التالي دائماً لضمان الدقة
            if i + 1 < len(table):
...
def convert_df_to_excel(df):
    ...
    columns_order = [
        ...
        'رسائل محلية',
        'مكالمات محلية',
        'رسوم الخدمات', # لاحظ أن ملف الإكسل قد لا يحتوي على عمود "خدمات" منفصل في بعض الصفوف، لكنه موجود في الهيدر
        'رسوم شهرية'
    ]
    
    # التأكد من وجود الأعمدة وترتيبها
    existing_cols = [col for col in columns_order if col in df.columns]
    # إضافة أعمدة مفقودة بقيم صفرية إذا لزم الأمر للحفاظ على الهيكل
