...
# ================= LOGIC (دمج منطق السالب وتنظيف الموبايل) =================
def normalize(t):
...
def extract_numbers(text):
    ...
    # معالجة الأقواس كأرقام سالبة وتصحيح المسافات قبل السالب
    text = re.sub(r'\((\d+\.?\d*)\)', r'-\1', text)
    text = re.sub(r'(\d+\.?\d*)-', r'-\1', text)
    text = re.sub(r'-\s+(\d)', r'-\1', text)
    # فصل الأرقام الملتصقة بعلامة السالب
                ...
                for table in tables or []:
                    for i, row in enumerate(table):
                        ...
                        if phone_match:
                            ...
                            if i + 1 < len(table):
                                nxt_text = normalize(" ".join([str(c) for c in table[i+1] if c]))
                                nxt_vals = extract_numbers(nxt_text)
                                if len(nxt_vals) > len(vals):
                                    vals = nxt_vals
                                    i += 1
                            
                            # تنظيف القائمة من رقم الموبايل لمنع تداخله مع المبالغ
                            # نحول رقم الموبايل لنص ونقارنه بالجزء الصحيح من الأرقام المستخرجة
                            vals = [v for v in vals if str(int(abs(v))) not in p]
                            
                            if is_arabic: vals = vals[::-1]
                            
                            def g(idx): return vals[idx] if idx < len(vals) else 0
                            
                            # منطق التسويات (إذا كان موجب نحوله لسالب بناءً على النسخة القديمة المظبوطة)
                            taswiya = g(10)
                            if taswiya > 0: taswiya = taswiya * -1
...
if st.button("🚀 بدء المعالجة والتحليل"):
    if files:
        ...
        if all_data:
            ...
            # --- تنظيف إضافي لمنع ظهور أجزاء من رقم الموبايل في الحسابات ---
            def clean_mobile_artifacts(row):
                mobile = str(row['محمول'])
                for col in df.columns:
                    if col != 'محمول':
                        val_str = str(int(abs(row[col])))
                        if val_str in mobile and len(val_str) > 4: # لو الرقم جزء كبير من الموبايل
                            row[col] = 0
                return row
            
            df = df.apply(clean_mobile_artifacts, axis=1)
            ...
            with m3: 
                t_sett = df["رسوم تسويات"].sum()
                color = "red" if t_sett < 0 else "#1a7e43"
                st.markdown(f'<div class="metric-card"><div class="metric-title">🧾 تسويات</div><div class="metric-value" style="color: {color};">{t_sett:,.1f}</div></div>', unsafe_allow_html=True)
            ...
            st.warning("لم يتم العثور على بيانات داخل الملفات.")
    else:
        st.info("يرجى رفع ملفات PDF للبدء.")

# التوقيع
st.markdown("<div style='text-align:center; margin-top:40px; color:#666;'>Developed by Najat El Bakry © 2026</div>", unsafe_allow_html=True)
