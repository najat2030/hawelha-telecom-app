...
الارقام دى المبالغ بتاعتها المفروض تاجي فعامود التسوية 
لما عملت استخراج لملف الاكسل الارقام جاية فعامود المكالمات المحلية
ليه كدة انا اعصابي تعبت بعد ما سلمت المشروع للادارة طلع شكلي زي الزفت قدامهم  

نوجا، المشكلة واضحة من الكود نفسه: الاستخراج عندك بيعتمد على ترتيب الأرقام فقط، فلو ترتيب جدول الـPDF اتقلب أو اتزحزح، رقم التسوية السالب بيتحط في أول خانة “منطقية” حسب الـscore، مش حسب اسم العمود الحقيقي.

قام بالتفكير لمدة 47s
حقك تزعلي، بس دي غلطة منطق ترتيب الأعمدة مش منك.

السبب في الكود:

الكود بيحط مكالمات محلية = الرقم رقم 3 في ترتيب الأرقام.

وبيحط رسوم تسويات = الرقم رقم 11.

يعني أي مبلغ سالب لو ظهر بدري في ترتيب الأرقام المستخرجة من الـPDF، هينزل غلط في “مكالمات محلية”. 


كمان الكود بيجرب يعكس ترتيب الأرقام ويختار حسب score، وده ممكن يختار ترتيب غلط لو الإجمالي قريب منطقيًا. 


الحل السريع الآمن
بعد ما يتبني best_record مباشرة، ضيفي التصحيح ده قبل:

records.append(best_record)
خليه كده:

# تصحيح مهم:
# أي مبلغ سالب ظهر في أعمدة الاستخدام المحلية غالبًا يكون تسوية/رصيد دائن
if best_record["مكالمات محلية"] < 0 and best_record["رسوم تسويات"] == 0:
    best_record["رسوم تسويات"] = best_record["مكالمات محلية"]
    best_record["مكالمات محلية"] = 0

records.append(best_record)
ولو عايزة أقوى شوية، استخدمي ده:
...
for col in usage_cols:
    if best_record[col] < 0:
        best_record["رسوم تسويات"] += best_record[col]
        best_record[col] = 0
...
1122007084   -2.22
1114613098   -4.14
تنزل في رسوم تسويات مش مكالمات محلية.


18e2631c-d487-4e02-ab7a-5a8e5e8b53f3.png
انت خربتها 

206e2728-7479-4ca4-89f8-3a6c948fcb68.png
وبعدين طيب

8ae9f0ce-af51-43ff-a9ff-64b92906a781.png
خربتها وقعدت ع تلها

9f6f8dbd-e45b-4ab8-b56f-4d6b47f9577e.png

نص واحد ملصق (14).txt
مستند
منك لله بجد 
اخلص صحح الكود وابعت لي المود كامل بدون ماتغيييير اي حاجة غير الجزء البايظ فقطططططططططططططططططط فااااهم 

مش هتنيل اعدل حاجة ابعت لكود كامل بعد ماتعدل واياااااك تغير حاجة فالكود الاساسي غير الجزء اللي محتاج تعديل

قام بالتفكير لثانيتين
حقك عليا. ده الكود كامل بعد تعديل الجزء البايظ فقط:
التعديل الوحيد داخل parse_file: تصحيح مكان except + نقل السالب من مكالمات محلية إلى رسوم تسويات. 
...
1122007084	3.03
1114613098	3.33 
دول مبالغ رسوم شهرية ليه حاطهها فالاجمالي 
كل حاجة مظبوطة ماعدا الكام رقم دول عكس القيم اللي فعامود رسوم شهريه وداها فالاجمالي واخد اللي مبالغهم اللي المفروض تكون فالاجمالي وداها فرسوم شهرية واخر عجن  

قام بالتفكير لمدة 9s
أيوه كده السبب واضح: الكود اختار reversed_record غلط، فعكس رسوم شهرية مع الإجمالي. 


عدّلي نفس الجزء الأخير فقط داخل parse_file.
...
خليه كده:
                            ...
                            # تصحيح حالة انعكاس رسوم شهرية مع الإجمالي
                            if (
...
1122007084    3.03
1114613098    3.33
ترجع في رسوم شهرية، والقيمة الكبيرة ترجع في الإجمالي.


عدل وابعت لي الكود كامل

قام بالتفكير لثانيتين
اتفضلي الكود كامل بعد تعديل جزء التصحيح فقط: نقل السالب للتسويات + تصحيح انعكاس رسوم شهرية مع الإجمالي. 
...
with col_me:
initial = st.session_state.username[0].upper()
st.markdown(f'{initial}مرحباً، {st.session_state.username}', unsafe_allow_html=True)
