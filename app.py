...
def extract_values_from_row(row):
    """
    استخراج القيم من الصف بنفس الترتيب الموجود في ملف الـ PDF
    ...
    # بنقرأ الخلايا بنفس الترتيب عشان نطابق الترتيب العربي (من اليمين لليسار)
    for cell in reversed(row):
        if not cell:
            continue
        cell_text = str(cell).strip()
        
        # استخراج الأرقام مع حفظ الإشارة (سالب أو موجب)
